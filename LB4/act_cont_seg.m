...
%
% seg = act_cont_seg(I,init_mask,max_its,alpha,display)
%
% Inputs: I           2D image
...
% seg = act_cont_seg(img,m,500);
%
% Copyright by Shawn Lankton (www.shawnlankton.com)
%------------------------------------------------------------------------
function seg = act_cont_seg(I,init_mask,max_its,alpha,display, showIterStep)
  ...
  for its = 1:max_its   % Note: no automatic convergence test
    ...
    %-- intermediate output
    if((display>0)&&(mod(its, showIterStep) == 0)) 
